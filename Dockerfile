...
RUN apt-get update \
  ...
  && LC_ALL=ja_UJP.UTF-8 add-apt-repository ppa:ondrej/php \
  && apt-get update \
  ...
  && echo "date.timezone = Asia/Tokyo" >> /etc/php/$php_version/apache2/php.ini \
  && sed -i -e 's/display_errors = .*/display_errors = On/' -e 's/log_errors = .*/log_errors = On/' /etc/php/$php_version/apache2/php.ini \
  ...
  && echo "error_log = /home/${user}/www/hotelcommerce/php-errors.log" >> /etc/php/$php_version/apache2/php.ini \
  && apt-get install -y git nano vim curl openssh-server \
  ##setup non root user
  && useradd -m -s /bin/bash ${user} \
  && mkdir -p /home/${user}/www/ 
##Download Qloapps latest version
COPY ./hotelcommerce/ /home/${user}/www
